// adapters/rust-persistent.ts — persistent-process Rust adapter.
//
// Same Adapter contract as adapters/rust.ts, but instead of spawning a fresh
// process per call it starts ONE `toon-bridge serve` process and speaks
// newline-delimited JSON to it. For a big sweep this turns ~15k process spawns
// into one. It is the Rust analogue of adapters/python-persistent.ts, and its
// behavior parity with the one-shot adapter is PROVEN by
// adapters/selftest-parity-rust.ts before any large run:
...
// before the next, so at most one request is ever in flight; the FIFO queue
// below keeps responses matched to callers even if that ever stops being true.
...
function ensureChild(): ChildProcessWithoutNullStreams {
  ...
  c.stdout.on("data", (chunk: string) => {
    ...
    while ((nl = buf.indexOf("\n")) >= 0) {
      ...
      try {
        ...
        else p.reject(new Error("rust worker: " + (resp.error ?? "unknown error")));
      } catch (e) {
  ...
  c.on("exit", (code, signal) => {
    if (!dead) die(`rust serve process exited (code=${code}, signal=${signal})`);
  });
  c.on("error", (e) => { if (!dead) die(`rust serve process error: ${e.message}`); });
...
function request(op: "encode" | "decode", data: string): Promise<string> {
  if (dead) return Promise.reject(new Error("rust serve adapter is dead"));
  const c = ensureChild();
...
export function shutdownRust(): void {
  if (child) {
